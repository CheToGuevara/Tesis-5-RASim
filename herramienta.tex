...
En la figura \ref{fig:posui} se muestra la interfaz d\del{ó}nde el usuario puede manejar las articulaciones que disponga el paciente virtual para conseguir la posición deseada. Se divide en las siguientes partes: 
\begin{enumerate}
    ...
    \begin{enumerate}
   \item Guarda\new{r} una instantánea de la escena que el usuario está viendo. 
   \item Activa\new{r} la fase de optimización a demanda del usuario. 
   \item Activa\new{r} la animación precargada que puede ser utilizada para seleccionar una pose. 
   \item Permit\del{e}\new{ir} devolver la cámara de la escena a su posición original. 
   \item Permit\del{e}\new{ir} utilizar la tecnología \emph{Microsoft Kinect} para posicionar el modelo anatómico.
   \item   Permit\del{e}\new{ir} al usuario terminar con el posicionamiento y cerrar la aplicación.
    \end{enumerate}
...
Hay que destacar que no se integró la tarea 3.4 que se encargaba de la descripción fisiológica. Se decidió no incluir la simulación de la técnica de estimulación eléctrica del nervio en el simulador \ac{RASim}\todo{más preciso}.

El proceso comienza con el módulo de registro diseñado por \ac{UKA-IMI}. Se procede a cargar una imagen médica que contiene la anatomía objetivo, procedente de una base de datos que almacena diferentes\todo{no concuerda en número} \ac{DICOM}, además del modelo superficial anatómico de referencia en formato \ac{X3D}. Después, se realiza un registro automático entre la imagen médica y el modelo comercial, dando como resultado un nuevo modelo que será el primer paso para crear un \ac{VPH}. Para más información se puede consultar en \cite{deOliveira:2015}. Este resultado será la entrada de la siguiente etapa.
